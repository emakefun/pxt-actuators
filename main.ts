...
//% color="#EE6A50" weight=10 icon="\uf085" block="actuators"
namespace actuators {
    ...
    //% weight=70
    export function actuator_buzzer1(pin: DigitalPin,status: on_off): void {
        pins.digitalWritePin(pin,status)
    }
	 ...
	 */
	 ...
	 let _INA=0
	 let _INB=0
    //% blockId=actuator_motor_pin block="actuator_motor_pin|ina %INA|inb %INB"   group="直流电机"
    //% weight=70
    export function actuator_motor_pin(INA: DigitalPin,INB: DigitalPin): void {
        _INA=INA
		_INB=INB
    }
	//% blockId=actuator_motor_run block="actuator_motor_run  %turn"  group="直流电机"
    //% weight=70
    export function actuator_motor_run(turn: run_turn): void {

        if (turn == 0){
			pins.digitalWritePin(_INA,0)
			pins.digitalWritePin(_INB,1)
			
		}else if (turn == 1){
			pins.digitalWritePin(_INA,1)
			pins.digitalWritePin(_INB,0)
		}
